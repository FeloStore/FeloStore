...
import app.FeloStore.client.BuildConfig
...
fun PackageManager.getPackageInstallStatus(appId: String, versionCode: Long?): InstallStatus {
    return try {
        ...
        //
        // 1. The installer of record is not FeloStore
        // 2. The installer of record is not null
        // 3. The package is not FeloStore
        //
        // Note that FeloStore never considers itself installed by another source. This behavior
        // ensures that FeloStore is always responsible for updating itself.
        if (
